...
uint8_t audio_data[1500];
uint32_t audioSize=0;
uint8_t datatype = 0;

// BLE Server callback when a new ble device is connected 
void MyServerCallbacks::onConnect(BLEServer* pServer) {
    deviceConnected = true;
    BLEAdvertising *pAdvertising = pServer->getAdvertising();
    pAdvertising->stop();
}
// BLE Server callback when a new ble device is disconnected 
void MyServerCallbacks::onDisconnect(BLEServer* pServer) {
    deviceConnected = false;
}

// BLE function to read delay time of ble for adaptive data rate.
void CharacteristicCallbacks::onWrite(BLECharacteristic *pCharacteristic1) {
...
// Function to take capture an false image to stabilise camera
void false_click(){
...
// Initialization code common to all sketches
void setupCommon() {  
    Serial.begin(115200);
    delay(10);
    ...
    // Frame parameters to set image quality
    if(psramFound()){
    ...
    // Initializing wifi soft access point to connect to pc for receiving audio files
